...
\integrante{Pellitero, Agustin}{002/01}{email2@dominio.com}
...
\begin{proc}{caminoMinimo}{\In origen: \ent, \In destino: \ent, \In distancias: \TLista{\TLista{\ent}}}{\TLista{\ent}}
	...
	(secuenciaEsCamino(distancias, s_1, origen, destino) \land secuenciaEsCamino(distancias, s_2, origen, destino)) \\
...
I \cong 0 \leq i \leq |\text{ciudades}| \land \sum_{j=0}^{i-1} \text{ciudades}[j].habitantes = res
...
\[
wp(S1, wp(i:=i+1, |\text{ciudades}|- i - 1 < v_0)) =
\]

\[
wp(S1, True \land |\text{ciudades}| - i - 2 < v_0) =
\]

\[
wp(res:= res + \text{ciudades}[i].habitantes, True \land |\text{ciudades}| - i - 2 < v_0) = 
\]

\[
True \land|\text{ciudades}| - i - 1 < v_0 =
\]

\[
 |\text{ciudades}| - i - 1 < v_0
\]
