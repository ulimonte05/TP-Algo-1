...
\begin{proc}{hayCamino}{\In distancia: \TLista{\TLista{\ent}}, \In desde: \ent, \In hasta: \ent}{\bool}
	\requiere{esCuadrada(distancia) \yLuego filaIgualColumna(distancia) \yLuego diagonalCero(distancia)}
...
\pred{diagonalCero}{\In mat: \TLista{\TLista{\ent}}}{
	\paraTodo[unalinea]{i,j}{\ent}{(0 \leq i,j < |mat| \land i = j) \implicaLuego mat[i][j] = 0
}}
...
\begin{proc}{cantidadCaminosNSaltos}{\Inout conexion: \TLista{\TLista{\ent}}, \In n: \ent}{}
	\requiere{1 \leq n}
    \requiere{esCuadrada(conexion) \yLuego filaIgualColumna(conexion) \yLuego diagonalCero(conexion)}
...
\begin{proc}{caminoMinimo}{\In origen: \ent, \In destino: \ent, \In distancias: \TLista{\TLista{\ent}}}{\TLista{\ent}}
	\requiere{esCuadrada(distancias) \yLuego filaIgualColumna(distancias) \yLuego diagonalCero(distancias)}
...
Definimos Q_c \equiv ( \sum_{j=0}^{|\text{ciudades}|-1} \text{ciudades}[j].habitantes = res \land i = |\text{ciudades}|)
...
\[
\{0 \leq i \leq |\text{ciudades}| \yLuego \sum_{j=0}^{i-1} \text{ciudades}[j].habitantes = res \land 0 \leq i < |\text{ciudades}| \land F_v = |\text{ciudades}| - i\} \implica 
\]
...
\[
wp(S1, def(i := i + 1) \land |ciudades| - (i + 1) < v_0) \equiv
\]

\[
wp(S1, True \land |\text{ciudades}| - i - 1 < v_0) \equiv
\]

\[
wp(res:= res + \text{ciudades}[i].habitantes, |\text{ciudades}| - i - 1 < v_0) = 
\]

\[
def(res := res + ciudades[i].habitantes) \yLuego |ciudades| - i - 1 < v_0 \equiv
\]

\[
wp(S, F_v < v_0) \equiv 0 \leq i < |ciudades| \yLuego |ciudades| - i - 1 < v_0
\]
...
\[
\{0 \leq i \leq |\text{ciudades}| \yLuego \sum_{j=0}^{i-1} \text{ciudades}[j].habitantes = res \land 0 \leq i < |\text{ciudades}| \land |\text{ciudades}| - i\ = v_0} \implica 
\]

\[
0 \leq i < |ciudades| \yLuego |ciudades| - i - 1 < v_0 \equiv
\]
...
\[
0 \leq i \leq |\text{ciudades}| \yLuego res = \sum_{j=0}^{i-1} \text{ciudades}[j].habitantes \yLuego |\text{ciudades}| - i \leq 0 \implica i \geq |\text{ciudades}| \equiv
\]

\[
0 \leq i \leq |\text{ciudades}| \yLuego |\text{ciudades}| \leq i \implica |\text{ciudades}| \leq i \equiv
\]

\[
|\text{ciudades}| \leq i \leq |\text{ciudades}| \implica |\text{ciudades}| \leq i  \equiv
\]
